...
static unsigned get_bot_input1(const struct bot *const bot) {
        return get_bot_input(bot->highlow1, bot->input1);
}
static unsigned get_bot_input2(const struct bot *const bot) {
        return get_bot_input(bot->highlow2, bot->input2);
}
...
static void assign_input(bool to_output, unsigned idx,
                         struct entity *input, enum highlow highlow) {
        if (to_output) {
...
static void parse_move_line(const char **const input, unsigned *const ibot,
                            unsigned *const ilow, unsigned *const ihigh,
...
static void parse_assign_line(const char **const input,
                              unsigned *const value, unsigned *const ibot) {
