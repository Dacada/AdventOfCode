...
static void string_builder_append_array(struct string_builder *const str, const char *const array, const size_t size) {
  bool grow = false;
...
__attribute__((pure)) static size_t decompressed_length(const char *const text, size_t size) {
  size_t count = 0;
