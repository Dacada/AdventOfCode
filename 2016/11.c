...
static void set_true_for_packed_equivalent_states(const struct state *const state,
                                                  bool *const seen_states) {
...
static bool parse_element(const char **const input,
                          unsigned rock_floor[MAX_ROCKS], unsigned chip_floor[MAX_ROCKS],
...
static void parse_line(const char **const input,
                       unsigned rock_floor[MAX_ROCKS], unsigned chip_floor[MAX_ROCKS],
...
static bool is_state_finished(const struct state *const state) {
        ...
        for (unsigned i=0; i<total_rocks; i++) {
                if (state->rock_floor[i] != last_floor ||
                    state->chip_floor[i] != last_floor) {
                        return false;
                }
        }
        return true;
...
static unsigned find_fastest_path(const struct state *const init) {
        ...
        while (qhead != qtail) {
                ...
                size_t number_next_states =
                        populate_possible_next_states(state, possible_next_states);
