...
__attribute__((pure)) static bool istls(const char *const ip) {
  ...
  for (unsigned i = 0;; i++) {
    ...
    if (!found_abba || inbrackets) {
      if (prevchars[1] == prevchars[2] && prevchars[0] == c && prevchars[0] != prevchars[1]) {
        if (inbrackets) {
          return false;
        } else {
          found_abba = true;
...
__attribute__((pure)) static bool isssl(const char *const ip) {
  char abas[256][3];
  char babs[256][3];
...
static void solution1(const char *const input, char *const output) { solution(input, output, istls); }

static void solution2(const char *const input, char *const output) { solution(input, output, isssl); }
