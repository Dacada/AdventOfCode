...
static void iter_shares(const unsigned int elements, const unsigned int total, unsigned int *const share,
                        const size_t i, void (*const fun)(const unsigned int *const, void *const), void *const args) {
  if (elements > total) {
...
static unsigned int calc_share_value(const unsigned int *const share) {
  ...
  return ZERO_IF_NEG(total_capacity) * ZERO_IF_NEG(total_durability) * ZERO_IF_NEG(total_flavor) *
         ZERO_IF_NEG(total_texture);
}
