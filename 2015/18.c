...
static unsigned int neighbors(size_t i, size_t j) {
  ...
  if (i < GRIDSIZE - 1 && j < GRIDSIZE - 1 && lights[current][i + 1][j + 1])
    result++;
  if (i < GRIDSIZE - 1 && lights[current][i + 1][j])
    result++;
  if (i < GRIDSIZE - 1 && j > 0 && lights[current][i + 1][j - 1])
    result++;
  if (j < GRIDSIZE - 1 && lights[current][i][j + 1])
    result++;
  if (j > 0 && lights[current][i][j - 1])
    result++;
  if (i > 0 && j < GRIDSIZE - 1 && lights[current][i - 1][j + 1])
    result++;
  if (i > 0 && lights[current][i - 1][j])
    result++;
  if (i > 0 && j > 0 && lights[current][i - 1][j - 1])
    result++;
  return result;
}
