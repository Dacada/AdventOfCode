...
static void parse(char *input, unsigned int *containers) {
...
static void fits_exactly(int *containers_int, void *args_void) {
...
static unsigned int solve1(unsigned int *containers) {
  struct args_t args;
  args.total = 0;

  for (size_t ncomb=1; ncomb<=NCONTAINERS; ncomb++) {
    args.len = ncomb;
    aoc_combinations((int*)containers, NCONTAINERS, ncomb, fits_exactly, &args);
  }

  return args.total;
}

static unsigned int solve2(unsigned int *containers) {
  struct args_t args;
  args.total = 0;
