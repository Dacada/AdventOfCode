...
static int *get_remainder(const int *const array, size_t len, const int *const subarray, size_t sublen,
                          size_t *const rlen) {
  *rlen = len - sublen;
...
static bool optimal_arrangement(const int *const numbers, size_t len, int **const groups, size_t *const lens,
...
static void find_arrangement(int *group, void *vargs) {
  ...
  *(args->lens) = args->group_len;
  ...
  if (optimal_arrangement(remainder, remainder_len, args->groups + 1, args->lens + 1, args->ngroups - 1, args->goal)) {
    args->found = true;
  }
  free(remainder);
}
...
static void solution1(const char *const input, char *const output) { solution(input, output, 3); }

static void solution2(const char *const input, char *const output) { solution(input, output, 4); }
