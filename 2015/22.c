...
static void optimum_info_update(struct optimum_info *const optimum, const struct state *const state) {
        ...
        if (state->mana_spent <= optimum->mana) {
                ...
                memcpy(optimum->spell_log_list[optimum->spell_log_list_len],
                       state->spell_log, state->spell_log_len * sizeof(enum spell));
...
static void demo_print_player_boss(const struct state *const state) {
        ...
        DMO("- Player has %u hit points, %u armor, %u mana.",
            state->player_hp, state->effect_shield_turns_left > 0 ? 7 : 0, state->player_mana);
...
static enum turn_result run_effects(bool hard_mode,
...
static enum turn_result player_cast(struct state *const state,
                                    enum spell spell) {
        #ifdef DEBUG
...
static enum turn_result run_player_turn(struct state *const state,
                                        enum spell spell) {
...
static enum turn_result run_boss_turn(const struct constant_state *const constants,
                                      struct state *const state) {
        if (state->effect_shield_turns_left > 0) {
...
static void run_turn(const struct constant_state *const constants,
                     struct state *const oldstate,
