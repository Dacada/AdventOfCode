        ...
        for (size_t ring2 = 0; ring2 < rings_len; ring2++) {
          if ((ring1 == 0 && ring2 == 0) || (ring1 != ring2)) {
            ...
            if ((win && vic) || (!win && !vic)) {
              ...
              if ((minimize && cost < best_cost) || (!minimize && cost > best_cost)) {
                best_cost = cost;
              }
            }
...
static void solution1(const char *const input, char *const output) { solution(input, output, true, true); }

static void solution2(const char *const input, char *const output) { solution(input, output, false, false); }
