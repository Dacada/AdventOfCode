...
static void stack_reverse(struct stack *const stack) {
        reverse_array(stack->elements, stack->head+1);
}
...
static void solution1(const char *const input, char *const output) {
        solution(input, output, apply_instruction_1);
}

static void solution2(const char *const input, char *const output) {
        solution(input, output, apply_instruction_2);
}
