...
static inline bool could_make_robot(enum robots robot,
                                    const struct blueprint *blueprint, const struct state *state, int minutes) {
        switch (robot) {
        case ROBOT_ore:
                return state->ore_robots * minutes + state->ore < blueprint->max_ore * minutes &&
