...
static void follow_instructions(struct state *const state, const char *map, const struct instruction *instr, const int width, const int height, const int len, bool cube) {
...
static inline int password(const struct state *s) {
        return 1000*(s->y+1) + 4*(s->x+1) + s->facing;
}
