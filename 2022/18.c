...
static int search(const bool *cube_map, int dim) {
  ...
  while (stack_len > 0) {
    ...
    for (unsigned i = 0; i < sizeof(cardinals) / sizeof(*cardinals); i++) {
      ...
      if (newp.x < 0 || newp.x >= dim || newp.y < 0 || newp.y >= dim || newp.z < 0 || newp.z >= dim) {
        continue;
      }
      ...
      if (cube_map[IDX(newp, dim)]) {
        if (newp.x == 3 && newp.y == 3 && newp.z == 2)
          DBG("collided with cube %d,%d,%d from %d,%d,%d by adding %d,%d,%d", newp.x, newp.y, newp.z, p.x, p.y, p.z,
              cardinals[i].x, cardinals[i].y, cardinals[i].z);
        total++;
        continue;
      }
