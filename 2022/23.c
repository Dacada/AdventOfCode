...
static bool spread(bool *points) {
  ...
  for (int j = 0; j < DIMENSION; j++) {
    for (int i = 0; i < DIMENSION; i++) {
      ...
      DBG("elf at %d,%d has proposed to move %s (to %d,%d)", i, j,
          ((const char *[]){"north", "south", "west", "east"})[proposal_dir], x, y);
    }
  }
  ...
  for (int j = 0; j < DIMENSION; j++) {
    for (int i = 0; i < DIMENSION; i++) {
      ...
      if (elf_next_position[IDX(x, y)] > 1) {
        DBG("elf at %d,%d wanted to move %s to %d,%d but cannot as other elves also want to move there", i, j,
            ((const char *[]){"north", "south", "west", "east"})[next_dir], x, y);
        new_points[IDX(i, j)] = true;
        continue;
      }
