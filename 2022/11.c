...
#if defined(DEBUG) && defined(DBGLOG)
static int dbg_number_to_int(const struct number *n) {
        return n->n;
}
#endif
...
static int inspect_item(struct monkey *monkey, int i) {
        ...
        MONKEYLOG("    Monkey gets bored with item. "
                  "Worry level is divided by 3 to %d", dbg_number_to_int(item));
...
static void solution(const char *const input, char *const output, int turns) {
        ...
        for (int turn=0; turn<turns; turn++) {
                ...
                if (dbgPrnt) DBG("== After round %d ==", turn);
                for (int i=0; i<len; i++) {
                        totals[i] += monkey_turn(monkeys, i);
                        if (dbgPrnt) DBG("Monkey %d inspected items %d times.", i, totals[i]);
                }
                if (dbgPrnt) DBG(" ");
        }
