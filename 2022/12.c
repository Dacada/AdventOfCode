...
static bool queue_empty(const struct queue *q) { return q->head == q->tail; }
...
static void populate_cardinals(struct point cardinals[4], struct point p, int width, int height, const int *map,
                               int *memory) {
  for (int i = 0; i < 4; i++) {
    cardinals[i] = p;
  }
  ...
  for (int i = 0; i < 4; i++) {
    ...
    if (cardinal.x < 0 || cardinal.x >= width || cardinal.y < 0 || cardinal.y >= height) {
      cardinals[i] = invalid;
      continue;
    }
...
static int bfs(const int *map, int width, int height, struct point start, struct queue *q, int *memory) {
  ...
  while (!queue_empty(q)) {
    ...
    for (int i = 0; i < 4; i++) {
      ...
      // DBG("%d,%d (%d) -> %d,%d (%d)", here.x, here.y, map[IDX(here, width)], cardinal.x, cardinal.y,
      // map[IDX(cardinal, width)]);
