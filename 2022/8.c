...
static int get_tree_score(int idx, int *trees, struct tree_info *info) {
  int tree = trees[idx];
  return info[idx].up.count[tree] * info[idx].down.count[tree] * info[idx].left.count[tree] *
         info[idx].right.count[tree];
}

static void send_line(int point, bool start, bool vertical, int *trees, bool *visible, struct tree_info *info,
                      int len) {
  int init;
  int end;
  int inc;
