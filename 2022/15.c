...
static int segment_length(struct segment x) { return x.end - x.start; }

static bool segment_contains(struct segment x, int p) { return p >= x.start && p <= x.end; }
...
static void line_segment_collection_free(struct line_segment_collection *c) { free(c->segments); }
...
static int distance_points(struct point a, struct point b) { return abs(a.x - b.x) + abs(a.y - b.y); }
...
static int radius(const struct circle *c) { return distance_points(c->center, c->point); }
