...
static int best_path(int len, const int *flow_rates, const int *distances,
                     int minutes, int current, const bool *open_valves) {
        int best = 0;
        for (int i=0; i<len; i++) {
                if (open_valves[i]) {
                        continue;
...
static int best_path_2(int len, const int *flow_rates, const int *distances, const int *starting_distances,
                       int minutes, int current, const bool *open_valves, bool restarted) {
        int best = 0;
        for (int i=0; i<len; i++) {
                if (open_valves[i]) {
                        continue;
                ...
                int pressure = released_pressure + best_path_2(len, flow_rates, distances, starting_distances, minutes_left, i, new_open_valves, restarted);
                free(new_open_valves);
        ...
        if (!restarted) {
                for (int i=0; i<len; i++) {
                        ...
                        int pressure = released_pressure +
                                best_path(len, flow_rates, distances, minutes_left, i, new_open_valves);
                        free(new_open_valves);
                        if (pressure > best) {
                                best = pressure;
                        }
...
static int best_ordering_pressure_2(const int *flow_rates, const int *distances, const int *starting_distances, int len) {
        int best = 0;
        for (int i=0; i<len; i++) {
                int minutes_to_travel = starting_distances[i];
                int minutes_left = 26 - minutes_to_travel - 1;
                ...
                memset(open_valves, 0, sizeof(*open_valves)*len);
                open_valves[i] = true;
                int pressure = released_pressure + best_path_2(len, flow_rates, distances, starting_distances, minutes_left, i, open_valves, false);
                free(open_valves);
                if (pressure > best) {
                        best = pressure;
                }
        ...
        for (int i=0; i<len; i++) {
                ...
                for (int j=0; j<len; j++) {
                        ...
                        fprintf(stderr, "Valve %c%c is at distance %d from valve %c%c\n",
                                vi/26+'A', vi%26+'A', distances[IDX(i,j,len)], vj/26+'A', vj%26+'A');
                }
        }
#endif
...
static void solution1(const char *const input, char *const output) {
        solution(input, output, best_ordering_pressure);
}
