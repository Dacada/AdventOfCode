...
static void linked_list_free(struct linked_list *list) { free(list->first); }
...
static long remove_wraparounds(long advance, int len) { return advance % (len - 1); }
...
static void solution1(const char *const input, char *const output) { solution(input, output, 1, 1); }

static void solution2(const char *const input, char *const output) { solution(input, output, 811589153, 10); }
