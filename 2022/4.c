...
static bool fully_contained(const struct pair *const pair) {
  return range_fully_contained(&pair->elf1, &pair->elf2) || range_fully_contained(&pair->elf2, &pair->elf1);
}
...
static bool line_overlap(const struct range *const range1, const struct range *const range2) {
  return (range1->start >= range2->start && range1->start <= range2->end) ||
         (range1->end >= range2->start && range1->end <= range2->end) || range_fully_contained(range2, range1);
}

static bool overlaps(const struct pair *const pair) { return line_overlap(&pair->elf1, &pair->elf2); }
...
static void solution1(const char *input, char *const output) { solution(input, output, fully_contained); }

static void solution2(const char *const input, char *const output) { solution(input, output, overlaps); }
