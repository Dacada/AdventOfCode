...
__attribute__((pure)) static int find_cycle(int cycle, int *cycles, int len) {
  if (len <= 0) {
    FAIL("logic error");
  }
  if (len == 1) {
...
__attribute__((pure)) static int signal_value(int *cycles, int *values, int len, int cycle) {
