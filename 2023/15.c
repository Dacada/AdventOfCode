...
static void hash_init(struct hash *this) { this->value = 0; }
...
static int hash_result(const struct hash *this) { return this->value; }
...
static void charstring_free(struct charstring *this) { free(this->str); }
...
static void charstring_clear(struct charstring *this) { this->len = 0; }
...
static void lens_free(struct lens *this) { charstring_free(&this->label); }
...
__attribute__((pure)) static struct lens *box_get_lens(struct box *this, const struct charstring *label) {
...
static void hashmap_operation(struct hashmap *hashmap, const struct charstring *label, char operation,
                              int focal_length) {
...
static int hashmap_result(const struct hashmap *hashmap) {
  ...
  while ((box = hashmapiterator_next(&hashmap_iter)) != NULL) {
    ...
    while ((lens = boxconstiterator_next(&box_iter)) != NULL) {
      ...
      DBG("'%s': '%d' (box %d) * '%d' (%d slot) * '%d' (focal length) = '%d'", charstring_tmpstr(&lens->label), i + 1,
          i, j + 1, j + 1, lens->focal_length, r);
      res += r;
      j++;
    }
    i++;
