...
static struct state stack_pop(struct stack *stack) {
...
static int direct_distance(const char *map, int width, int height, struct point p1, struct point p2) {
	...
	while (!queue_empty(&q)) {
		...
		for (int i=0; i<4; i++) {
			...
			if (can.x < 0 || can.x >= width ||
			    can.y < 0 || can.y >= height) {
				continue;
			}
			if (IDX(map, can.x, can.y, width) == '.' &&
			    !IDX(visited, can.x, can.y, width)) {
				if (found) {
					foundmany = true;
					break;
				} else {
...
static bool path_contains(const struct path *p, int node) {
	return p->contains[node];
}

static int path_latest(const struct path *p) {
	return p->last_added;
}
