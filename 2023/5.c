...
static long range_index(const struct range *r, long idx) { return r->start + idx; }

static bool range_overlap(const struct range *r1, const struct range *r2, struct range *left, struct range *right,
                          struct range *overlap) {
...
static void range_collection_free(struct range_collection *c) { free(c->ranges); }

static void range_collection_subdivide(struct range_collection *current, struct range input, struct range output,
...
static void solution2(const char *input, char *const output) {
  ...
  for (int j = 0; j < NMAPS; j++) {
    ...
    for (int k = 0; k < map->nranges; k++) {
      ...
      range_collection_subdivide(&element_ranges, map->input_ranges[k], map->output_ranges[k], &next_element_ranges,
                                 &unsuccessfully_mapped_ranges);
