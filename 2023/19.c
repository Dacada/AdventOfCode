...
static void workflow_free(struct workflow *w) {
	free(w->rules);
}
...
static bool parse_rule(const char **input, struct rule *rule) {
	enum trait t;
	switch (**input) {
	case 'x': t = EXTREME; break;
	case 'm': t = MUSICAL; break;
	case 'a': t = AERODYNAMIC; break;
	case 's': t = SHINY; break;
	default: return false;
	}

	enum operation op;
	switch (*(*input + 1)) {
	case '<': op = LESSER_THAN; break;
...
static void parse_input(const char *input, struct workflow **workflows, int *nworkflows, struct part **parts, int *nparts) {
...
static long segment_len(struct segment s) {
...
static void apply_rule_to_possible_part(struct rule *rule, struct possible_part *obeys, struct possible_part *disobeys) {
...
__attribute__((pure))
static long accepted_possible_part_size_from_id(int id, struct possible_part part, struct workflow **index) {
	if (id == ID_ACCEPT) {
