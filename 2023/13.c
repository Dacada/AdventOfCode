...
static int get_reflection_vertical(const struct pattern *pattern, int forbidden_value) {
	...
	for (int i=0; i<pattern->width; i++) {
		...
		if (i != forbidden_value &&
		    strncmp(curr, prev, pattern->height) == 0 &&
		    verify_reflection_vertical(pattern, i)) {
			result = i;
			goto end;
		}
		char *tmp = prev;
...
static int get_reflection_horizontal(const struct pattern *pattern, int forbidden_value) {
	...
	for (int j=0; j<pattern->height; j++) {
		...
		if (j != forbidden_value &&
		    strncmp(curr, prev, pattern->width) == 0 &&
		    verify_reflection_horizontal(pattern, j)) {
			result = j;
			goto end;
		}
		char *tmp = prev;
