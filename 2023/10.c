...
__attribute__((pure)) static enum direction dir_tick_cw(enum direction d) {
  switch (d) {
  case NORTH:
    return NORTHEAST;
  case NORTHEAST:
...
__attribute__((pure)) static enum direction dir_tick_ccw(enum direction d) {
  switch (d) {
  case NORTH:
    return NORTHWEST;
  case NORTHWEST:
...
static bool point_eq(struct point p1, struct point p2) { return p1.x == p2.x && p1.y == p2.y; }

__attribute__((pure)) static struct point point_advance(struct point p, enum direction d) {
  struct point n = p;
  switch (d) {
  case NORTH:
    n.y -= 1;
...
__attribute__((pure)) static enum direction opposite(enum direction dir) {
  switch (dir) {
  case NORTH:
    return SOUTH;
  case SOUTH:
...
__attribute__((pure)) static bool is_direction_valid_to_connect(enum pipe p, enum direction dir) {
  switch (p) {
  case VERTICAL:
...
static void pipe_next(enum pipe *maze, struct point current, struct point *next1, struct point *next2, int width,
                      int height) {
...
__attribute__((pure)) static enum direction follow_out_direction(enum pipe curr, enum pipe prev,
...
static void solution2(const char *const input, char *const output) {
  ...
  while (!point_eq(curr, outside)) {
    enum direction dir =
        follow_out_direction(IDX(maze, curr, width), IDX(maze, prev, width), IDX(maze_info, prev, width).out);
