...
static bool state_init_next(struct state current, int i, struct state *next, const int *map, int width, int height,
                            bool ultra) {
  int maxdir = 3;
  if (ultra) {
    maxdir = 10;
...
static int state_value(const struct state *s) { return s->loss_count; }
