...
static bool eq_frac(__int128 n1, __int128 d1, __int128 n2, __int128 d2) { return n1 * d2 == n2 * d1; }
...
#ifdef TRACE
static void point_print(const struct point *p) { fprintf(stderr, "%ld, %ld, %ld", (long)p->x, (long)p->y, (long)p->z); }
#endif
...
static bool line_intersect2d(const struct line *l1, const struct line *l2, struct point *intersection,
  ...
  for (int i = 0; i < nlines; i++) {
    for (int j = i + 1; j < nlines; j++) {
      ...
      if (line_intersect2d(&lines[i], &lines[j], &intersect, &den)) {
        ...
        double y = dynum / dden;
#endif
        if (cmpden(intersect.x, from, den) >= 0 && cmpden(intersect.x, to, den) <= 0 &&
            cmpden(intersect.y, from, den) >= 0 && cmpden(intersect.y, to, den) <= 0) {
#ifdef TRACE
...
static bool all_lines_collide(struct line *lines, int nlines, __int128 adjust_x, __int128 adjust_y,
                              struct point *intersection, __int128 *denominator) {
...
static void time_at_point(const struct line *l, const struct point *point, __int128 denominator, __int128 *num,
                          __int128 *det) {
  // t = (i.x/d - p.x)/v.x
  // t = i.x/(d*v.x) - p.x/v.x
...
static bool get_z(const struct line *l1, const struct line *l2, const struct point *intersection, __int128 denominator,
                  __int128 *znum, __int128 *zdet) {
  ...
  // ((p1.z*dt1*dt2)/(dt1*dt2) - (p2.z*dt1*dt2)/(dt1*dt2) + (nt1*v1.z*dt2)/(dt1*dt2) - (nt2*v2.z*dt1)/(dt1*dt2)) /
  // ((nt1*dt2)/(dt1*dt2) - (nt2*dt1)/(dt1*dt2))
...
static bool find_third_coordinate(struct line *lines, int nlines, __int128 adjust_x, __int128 adjust_y,
                                  const struct point *intersection, __int128 denominator, __int128 *pos_z) {
    ...
    if (get_z(&l1, &l2, intersection, denominator, &candidate_num, &candidate_den)) {
      if (current_candidate_init) {
        ...
        */
        ...
        // Sadly, I am getting integer overflows on this equality check even using 128 bit integers... So I'm doing this
        // instead:
  ...
  /*
  __int128 num = l.p.z*current_candidate_den*time_den + l.v.z*time_num*current_candidate_den -
  current_candidate_num*time_num;
      ...
      for (int negx = 0; negx <= 1; negx++) {
        ...
        for (int negy = 0; negy <= 1; negy++) {
          ...
          if (all_lines_collide(lines, nlines, adjust_x, adjust_y, &intersection, &denominator)) {
            DBG(" All lines have a 2D collision at t>=0 by adjusting velocity by %d,%d,X", (int)adjust_x,
                (int)adjust_y);
            DBG(" The intersection happens at %f,%f,Z", (double)intersection.x / (double)denominator,
                (double)intersection.y / (double)denominator);
