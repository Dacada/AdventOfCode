...
static void parse_instruction_2(const char **input, struct instruction *instruction) {
	...
	*input += 1;
	...
	switch (**input) {
	case '0': instruction->dir = 'R'; break;
	case '1': instruction->dir = 'D'; break;
	case '2': instruction->dir = 'L'; break;
	case '3': instruction->dir = 'U'; break;
	default: FAIL("invalid direction %c", **input);
	}
	*input += 1;
...
static struct instruction *parse_input(const char *input, int *length, void(*parse_instruction)(const char**, struct instruction*)) {
	int len = 0;
	int cap = 8;
...
static void solution(const char *const input, char *const output, void(*parse_instruction)(const char**, struct instruction*)) {
	int len;
	...
	for (int i=0; i<len; i++) {
		...
		case 'U': p.y -= instructions[i].dist; break;
		case 'L': p.x -= instructions[i].dist; break;
		case 'D': p.y += instructions[i].dist; break;
		case 'R': p.x += instructions[i].dist; break;
		}
		points[i+1] = p;
	}
...
static void solution1(const char *const input, char *const output) {
	solution(input, output, parse_instruction_1);
}

static void solution2(const char *const input, char *const output) {
	solution(input, output, parse_instruction_2);
}
