...
static void state_free(struct state *state) { free(state->beams); }
...
static void advance(const char *contraption, int width, int height, const struct state *state, struct state *new_state,
