...
static unsigned encode_name(const char **const input, bool *const big, bool *const start, bool *const end) {
        ...
        if (strncmp(*input, "start", 5) == 0) {
                ...
                *input += 1;
                ASSERT((*big  && (**input >= 'A' && **input <= 'Z')) ||
                       (!*big && (**input >= 'a' && **input <= 'z')), "parse error");
                if (*big) {
                        n += **input - 'A';
                } else {
                        n += **input - 'a';
...
static unsigned count_paths(const struct maze *const maze, unsigned from, const struct set *const visited, bool small_cave_visited_twice, char *dbg_path, int dbg_path_i) {
        unsigned count = 0;
