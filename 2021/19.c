...
static void vector_zero(struct vector *const v) {
        v->x = v->y = v->z = 0;
}
...
static void position_identity(struct vector *const p) {
        vector_zero(p);
}
...
static void vector_apply_rot(struct vector *const res, const struct vector *const orig, const struct rotation *const rot) {
        res->x = dice_getCoord(rot->front, orig);
        res->y = dice_getCoord(rot->up, orig);
        res->z = dice_getCoord(dice_getZ(rot->front, rot->up), orig);
}

static struct vector transform_vector(const struct vector *const orig,
                                      const struct vector *const pos, const struct rotation *const rot) {
        struct vector res;
...
static size_t find_overlaps(struct scanner *const scan1, struct scanner *const scan2,
                              unsigned **const match1, unsigned **const match2) {
...
static void calculate_relative(const struct scanner *const abs, const struct scanner *const rel,
                               ...
                               struct rotation *const relRot, struct vector *const relPos) {
...
static struct scanner *locate_scanners(const char *const input, size_t *len) {
        ...
        DBG("position of scanner 1 relative to scanner 0: %d,%d,%d", relativePositions[1*nscanners+0].x, relativePositions[1*nscanners+0].y, relativePositions[1*nscanners+0].z);
        DBG("rotation of scanner 1 relative to scanner 0: %u,%u", relativeRotations[1*nscanners+0].front, relativeRotations[1*nscanners+0].up);
        DBG("position of scanner 4 relative to scanner 1: %d,%d,%d", relativePositions[4*nscanners+1].x, relativePositions[4*nscanners+1].y, relativePositions[4*nscanners+1].z);
        DBG("rotation of scanner 4 relative to scanner 1: %u,%u", relativeRotations[4*nscanners+1].front, relativeRotations[4*nscanners+1].up);
        DBG("position of scanner 2 relative to scanner 4: %d,%d,%d", relativePositions[2*nscanners+4].x, relativePositions[2*nscanners+4].y, relativePositions[2*nscanners+4].z);
        DBG("rotation of scanner 2 relative to scanner 4: %u,%u", relativeRotations[2*nscanners+4].front, relativeRotations[2*nscanners+4].up);
        DBG("position of scanner 4 relative to scanner 2: %d,%d,%d", relativePositions[4*nscanners+2].x, relativePositions[4*nscanners+2].y, relativePositions[2*nscanners+4].z);
        DBG("rotation of scanner 4 relative to scanner 2: %u,%u", relativeRotations[4*nscanners+2].front, relativeRotations[4*nscanners+2].up);
                ...
                for (unsigned i=0; i<nscanners; i++) {
                        ...
                        for (unsigned j=0; j<nscanners; j++) {
                                ...
                                if (haveRelative[j*nscanners+i]) {
                                        ...
                                        calculate_absolute(&scanners[i], &scanners[j],
                                                           &relativePositions[j*nscanners+i], &relativeRotations[j*nscanners+i]);
                                        hasAbsolute[j] = true;
                                }
                        }
                }
        ...
        DBG("absolute position of scanner 0: %d,%d,%d", scanners[0].position.x, scanners[0].position.y, scanners[0].position.z);
        DBG("absolute rotation of scanner 0: %u,%u", scanners[0].rotation.front, scanners[0].rotation.up);
        DBG("absolute position of scanner 1: %d,%d,%d", scanners[1].position.x, scanners[1].position.y, scanners[1].position.z);
        DBG("absolute rotation of scanner 1: %u,%u", scanners[1].rotation.front, scanners[1].rotation.up);
        DBG("absolute position of scanner 2: %d,%d,%d", scanners[2].position.x, scanners[2].position.y, scanners[2].position.z);
        DBG("absolute rotation of scanner 2: %u,%u", scanners[2].rotation.front, scanners[2].rotation.up);
        DBG("absolute position of scanner 3: %d,%d,%d", scanners[3].position.x, scanners[3].position.y, scanners[3].position.z);
        DBG("absolute rotation of scanner 3: %u,%u", scanners[3].rotation.front, scanners[3].rotation.up);
        DBG("absolute position of scanner 4: %d,%d,%d", scanners[4].position.x, scanners[4].position.y, scanners[4].position.z);
        ...
        struct vector *beacons = malloc(sizeof(*beacons)*nbeacons_total);
        {
                ...
                for (unsigned i=0; i<nscanners; i++) {
                        ...
                        for (unsigned j=0; j<nbeacons; j++) {
                                beacons[k++] = transform_vector(&scanners[i].points[j],
                                                                &scanners[i].position, &scanners[i].rotation);
...
static void solution2(const char *const input, char *const output) {size_t nscanners;
        struct scanner *scanners = locate_scanners(input, &nscanners);
        ...
        for (size_t i=0; i<nscanners; i++) {
                for (size_t j=i+1; j<nscanners; j++) {
                        ...
                        if (distance > max) {
                                max = distance;
                                DBG("%lu (%d,%d,%d) %lu (%d,%d,%d)",
                                    i, scanners[i].position.x, scanners[i].position.y, scanners[i].position.z,
