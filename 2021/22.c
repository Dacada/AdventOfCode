...
static void cubeCollection_free(struct cubeCollection *const cubes) { free(cubes->cubes); }
...
static unsigned long volume_cuboid(const struct cuboid *const cube) {
  ...
  ASSERT(dx > 0 && dy > 0 && dz > 0, "invalid cuboid max=(%d,%d,%d) min=(%d,%d,%d)", cube->max.x, cube->max.y,
         cube->max.z, cube->min.x, cube->min.y, cube->min.z);
  return dx * dy * dz;
}

__attribute__((pure)) static unsigned long total_volume(const struct cubeCollection *const cubes) {
  long res = 0;
...
#define SET_X struct cuboid *x = newCubes->cubes + i
#define SET_XY                                                                                                         \
...
#define SET_XYZ                                                                                                        \
  ...
  *y = *x;                                                                                                             \
  *z = *x
#define VALIDATE(a) ASSERT(cV(a->min) <= cV(a->max), "invalid cuboid")
  ...
  for (size_t i = 0; i < l; i++) {
    ...
    // Amin       Amax  Bmin       Bmax
    // ~~~~~~~~~~~~     ------------
    if (cV(a->min) < cV(b->min) && cV(a->min) < cV(b->max) && cV(a->max) < cV(b->min) && cV(a->max) < cV(b->max)) {
      return false;
    }

    // Bmin       Bmax  Amin       Amax
    // ------------     ~~~~~~~~~~~~
    else if (cV(a->min) > cV(b->min) && cV(a->min) > cV(b->max) && cV(a->max) > cV(b->min) && cV(a->max) > cV(b->max)) {
      return false;
    }
    ...
    // ============------
    // XXXXXXXXXXXXYYYYYY
    else if (cV(a->min) == cV(b->min) && cV(a->min) < cV(b->max) && cV(a->max) > cV(b->min) &&
             cV(a->max) < cV(b->max)) {
      SET_XY;
    ...
    // X
      ...
      FAIL("impossible? (axis=%lu)\na: min=(%d,%d,%d) dimensions=(%d,%d,%d)\nb: min=(%d,%d,%d) dimensions=(%d,%d,%d)",
           axis, a->min.x, a->min.y, a->min.z, dim_a_x, dim_a_y, dim_a_z, b->min.x, b->min.y, b->min.z, dim_b_x,
           dim_b_y, dim_b_z);
#endif
    }
  }
...
static inline bool cuboid_collision(const struct cuboid *const a, const struct cuboid *const b,
                                    struct cubeCollection *newCubes) {
  ...
  for (size_t i = 0; i < newCubes->len; i++) {
    ...
    if (c->min.x >= a->min.x && c->min.x <= a->max.x && c->max.x >= a->min.x && c->max.x <= a->max.x &&
        c->min.y >= a->min.y && c->min.y <= a->max.y && c->max.y >= a->min.y && c->max.y <= a->max.y &&
        c->min.z >= a->min.z && c->min.z <= a->max.z && c->max.z >= a->min.z && c->max.z <= a->max.z) {
