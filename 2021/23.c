...
__attribute__((unused)) static void print_state(const struct state *const s) {
#ifdef DEBUG
  fprintf(stderr, "#############\n");
  fprintf(stderr, "#%c%c %c %c %c %c%c#\n", s->edges[0][1], s->edges[0][0], s->hall[0], s->hall[1], s->hall[2],
          s->edges[1][0], s->edges[1][1]);
...
static inline bool isagent(const char c) { return c == 'A' || c == 'B' || c == 'C' || c == 'D'; }
...
static bool edge2room_unblocked(const struct state *const state, int edge, int room) {
  ...
  return (r0 && ((e0) || (e1 && !h0 && !h1 && !h2))) || (r1 && ((e0 && !h0) || (e1 && !h1 && !h2))) ||
         (r2 && ((e0 && !h0 && !h1) || (e1 && !h2))) || (r3 && ((e0 && !h0 && !h1 && !h2) || (e1)));
}
...
static bool hall2room_unblocked(const struct state *const state, int hall, int room) {
  ...
  return (c0 && (r0 || r1 || (r2 && !h1) || (r3 && !h1 && !h2))) || (c1 && ((r0 && !h0) || r1 || r2 || (r3 && !h2))) ||
...
static bool is_room_valid(const struct state *const state, const char *room, const char c) {
  if (state->folded) {
    ...
    return !isagent(room[0]) && (!isagent(room[1]) || room[1] == c) && (!isagent(room[2]) || room[2] == c) &&
           (!isagent(room[3]) || room[3] == c);
  }
}
...
static bool is_deadlock_state(const struct state *const state) {
  ...
  for (int r = 0; r < 4; r++) {
    if (state->folded) {
      ...
      if (isagent(state->rooms[r][0]) || isagent(state->rooms[r][1]) || isagent(state->rooms[r][2]) ||
          !isagent(state->rooms[r][3])) {
        continue;
      }
...
#define NEXTSTATE                                                                                                      \
  ...
  *new = *state;                                                                                                       \
  new->count++
#define ENERGY(x) new->energy += (x)*energies[c - 'A']
...
static void queue_realloc(struct queue *const q) { q->nodes = realloc(q->nodes, q->size * sizeof(*q->nodes)); }
...
static bool queue_empty(struct queue *const q) { return q->len == 0; }
...
static void queue_free(struct queue *const q) { free(q->nodes); }
...
static void set_init(struct set *const h) { h->root = NULL; }
...
static void set_free(struct set *const h) { setNode_free(h->root); }
