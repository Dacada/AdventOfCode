...
static void apply_gravity_pair(int *arr, void *v_args) {
  ...
  if (args->axis == 'x')
    apply_gravity_axis(a->x, b->x, &va->x, &vb->x);
  if (args->axis == 'y')
    apply_gravity_axis(a->y, b->y, &va->y, &vb->y);
  if (args->axis == 'z')
    apply_gravity_axis(a->z, b->z, &va->z, &vb->z);
}
...
static int energy(struct vec3 v) { return ABS(v.x) + ABS(v.y) + ABS(v.z); }
...
__attribute__((pure)) static long lgcd(long a, long b) {
  int remainder = 0;
...
__attribute__((pure)) static long llcm2(long a, long b) {
  if (a == 0 || b == 0) {
    return 0;
  }
  return a * b / lgcd(a, b);
...
__attribute__((pure)) static long llcm3(long a, long b, long c) { return llcm2(llcm2(a, b), c); }
