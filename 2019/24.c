...
__attribute__((unused)) static void print_status(int minute, unsigned layouts[RECURSIONSIZE]) {
...
static unsigned advance(unsigned layout) {
  ...
  for (int j = 0; j < LAYOUTSIDE; j++) {
    for (int i = 0; i < LAYOUTSIDE; i++) {
      // count adjacent bugs
      int count = (i + 1 < LAYOUTSIDE && INDEXLAYOUT(i + 1, j, layout)) +
                  (i - 1 >= 0 && INDEXLAYOUT(i - 1, j, layout)) +
                  (j + 1 < LAYOUTSIDE && INDEXLAYOUT(i, j + 1, layout)) + (j - 1 >= 0 && INDEXLAYOUT(i, j - 1, layout));
...
static void advance2_rec(unsigned src[RECURSIONSIZE], unsigned dst[RECURSIONSIZE], size_t i, bool in, bool out) {
