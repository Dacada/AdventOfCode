...
static void handle_banned_item(char *const item) {
  static const char *const banned_items[NBANNEDITEMS] = {"giant electromagnet", "escape pod", "infinite loop",
                                                         "photons", "molten lava"};
...
static void parse_prompt(const char *const prompt, struct room *const room, char item[BUFFSIZE]) {
  ...
  ASSERT(isupper(prompt[i]), f, prompt[i]);
  i++;
  while (prompt[i++] != '\n')
    ;
  ASSERT(prompt[i] == '\n', f, prompt[i]);
...
static void clean_visited_counter(struct room *room) {
  ...
  if (room->north != NULL && room->north->times_visited)
    clean_visited_counter(room->north);
  if (room->south != NULL && room->south->times_visited)
    clean_visited_counter(room->south);
  if (room->east != NULL && room->east->times_visited)
    clean_visited_counter(room->east);
  if (room->west != NULL && room->west->times_visited)
    clean_visited_counter(room->west);
}
...
static long find_item_combination(struct IntCodeMachine *machine, char *item_names[8], struct room *room) {
  const char *go = NULL;
