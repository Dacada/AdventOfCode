  ...
  *columns = 0;
  ...
  while (machine->running) {
    ...
    if (out == '\n') {
      ...
      ASSERT(out == '.' || out == '#' || out == '^' || out == '<' || out == '>' || out == 'v',
             "Unexpected map tile: %ld", out);
      map[map_i] = out;
      map_i++;
    }
...
static void solution1(const char *const input, char *const output) {
  ...
  for (size_t j = 1; j < rows - 1; j++) {
    for (size_t i = 1; i < columns - 1; i++) {
      if (map[j * columns + i] == '#' && map[(j - 1) * columns + i] == '#' && map[(j + 1) * columns + i] == '#' &&
          map[j * columns + (i - 1)] == '#' && map[j * columns + (i + 1)] == '#') {
        result += i * j;
      }
    }
  }
...
static size_t get_path(char *const map, size_t rows, size_t columns, size_t x, size_t y, char *const movement) {
  ...
  } else if (valid_next_directions[(my_direction + 1) % 4] || valid_next_directions[(my_direction - 1) % 4]) {
...
static void get_starting_location(const char *const map, size_t rows, size_t columns, size_t *x, size_t *y) {
  for (size_t j = 0; j < rows; j++) {
    for (size_t i = 0; i < columns; i++) {
      char c = map[j * columns + i];
      if (c == '^' || c == '>' || c == 'v' || c == '<') {
