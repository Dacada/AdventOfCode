...
enum status { WALL = 0, FREE = 1, GOAL = 2 };
...
static void draw_map(const struct vec2 pos, int map[MAPSIZE][MAPSIZE], struct IntCodeMachine *const machine,
                     const struct vec2 start, const bool first) {
#ifdef DEBUG
  // show_map(map, pos);
  // usleep(50000);
#endif
