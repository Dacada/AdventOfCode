...
static bool queue_empty(struct queue *q) { return q->start == q->next; }
...
static char *parse_maze(const char *const input, size_t *const rows, size_t *const columns,
  ...
  *rows /= (*columns + 1);
  ...
  for (size_t i = 0; i < *columns; i++) {
    add_portal(portals, maze[INDEX(*columns, 0, i)], maze[INDEX(*columns, 1, i)], 1, i, true);
  }

  // Portals on the lower edge
  for (size_t i = 0; i < *columns; i++) {
    add_portal(portals, maze[INDEX(*columns, *rows - 2, i)], maze[INDEX(*columns, *rows - 1, i)], *rows - 2, i, true);
  }

  // Portals on the left edge
  for (size_t i = 0; i < *rows; i++) {
    add_portal(portals, maze[INDEX(*columns, i, 0)], maze[INDEX(*columns, i, 1)], i, 1, true);
  }

  // Portals on the right edge
  for (size_t i = 0; i < *rows; i++) {
    add_portal(portals, maze[INDEX(*columns, i, *columns - 2)], maze[INDEX(*columns, i, *columns - 1)], i, *columns - 2,
               true);
  }
  ...
  for (size_t i = hx; i < hx + lhx; i++) {
    add_portal(portals, maze[INDEX(*columns, hy, i)], maze[INDEX(*columns, hy + 1, i)], hy, i, false);
  }

  // Portals on the inner-lower edge
  for (size_t i = hx; i < hx + lhx; i++) {
    add_portal(portals, maze[INDEX(*columns, hy + lhy - 2, i)], maze[INDEX(*columns, hy + lhy - 1, i)], hy + lhy - 1, i,
               false);
  }

  // Portals on the inner-left edge
  for (size_t i = hy; i < hy + lhy; i++) {
    add_portal(portals, maze[INDEX(*columns, i, hx)], maze[INDEX(*columns, i, hx + 1)], i, hx, false);
  }

  // Portals on the inner-right edge
  for (size_t i = hy; i < hy + lhy; i++) {
    add_portal(portals, maze[INDEX(*columns, i, hx + lhx - 2)], maze[INDEX(*columns, i, hx + lhx - 1)], i, hx + lhx - 1,
               false);
  }
...
__attribute__((pure)) static struct vec2 advance(struct vec2 pos, int dir) {
  if (dir == 0) {
    pos.y -= 1;
    return pos;
  } else if (dir == 1) {
...
static struct vec2 step_out_portal(struct vec2 pos, char *maze, size_t columns) {
  ASSERT(ISPORTAL(maze[INDEX(columns, pos.y, pos.x)]), "Not stepping out of portal? %c at %lu,%lu",
         maze[INDEX(columns, pos.y, pos.x)], pos.x, pos.y);
  for (int d = 0; d < 4; d++) {
...
static int shortest_path(char *maze, size_t columns, struct portal portals[MAXPORTALS], struct vec2 from,
                         struct vec2 to, bool recursive, uint64_t **visited_levels) {
    ...
    for (int d = 0; d < 4; d++) { // for each direction
      ...
      if (c == '.') {                                // traversable ground
        ...
        if (p->entrance.x == next.x && p->entrance.y == next.y) {
          if (!recursive || (p->entrance_outer && level > 0) || !p->entrance_outer) {
            if (recursive) {
              if (p->entrance_outer) {
                nextlevel -= 1;
              } else {
          ...
          if (!recursive || (p->exit_outer && level > 0) || !p->exit_outer) {
            if (recursive) {
              if (p->exit_outer) {
                nextlevel -= 1;
              } else {
...
static void solution1(const char *const input, char *const output) {
  ...
  */
  ...
  int result = shortest_path(maze, columns, portals, portals[PORTALINDEX('A', 'A')].entrance,
                             portals[PORTALINDEX('Z', 'Z')].entrance, false, &visited);
...
static void solution2(const char *const input, char *const output) {
  ...
  int result = shortest_path(maze, columns, portals, portals[PORTALINDEX('A', 'A')].entrance,
                             portals[PORTALINDEX('Z', 'Z')].entrance, true, visited);
