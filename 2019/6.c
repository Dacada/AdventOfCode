...
#define MAX_PLANETS                                                                                                    \
  ((NUM_SYMBOLS - 1) + (NUM_SYMBOLS - 1) * NUM_SYMBOLS + (NUM_SYMBOLS - 1) * NUM_SYMBOLS * NUM_SYMBOLS)

#define SYMBOLTONUM(c) (c <= '9' ? c - '0' : c - 'A' + 10)

#define NAME2INDEX(name)                                                                                               \
  (SYMBOLTONUM(name[0]) + SYMBOLTONUM(name[1]) * NUM_SYMBOLS + SYMBOLTONUM(name[2]) * NUM_SYMBOLS * NUM_SYMBOLS)
...
static void parse(const char *input, int *planets) {
  while (*input != '\0') {
    ...
    input += 3;
    ASSERT(*input == ')', "Expected ) input but got %c %c%c%c[%c]%c%c%c", *input, input[-3], input[-2], input[-1],
