...
static void ensure_memory_size(struct IntCodeMachine *const machine, size_t index) {
        if (index > 1<<20) {
                fprintf(stderr, "I'm afraid I can't let you do that, Dave. (Refusing to allocate memory to access index %lu)", index);
                abort();
        }
...
static void solution(const char *const input, char *const output, long flag) {
        struct IntCodeMachine machine;
        machine_init(&machine, input);

        ASSERT(machine_send_input(&machine, flag), "Did not accept input?");
        machine_run(&machine);
        
        long o = -1;
        while (machine_recv_output(&machine, &o)) {
                //fprintf(stderr, "Ouput: %ld\n", o);
                machine_run(&machine);
        }
