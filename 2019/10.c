...
__attribute__((pure)) static bool visible(const int goalx, const int goaly, const int origx, const int origy,
...
static int get_best_position(const char *const map, const int rows, const int cols, int *const bestx,
                             int *const besty) {
  int best_visible = 0;
...
static int vec_cross(const int ax, const int ay, const int bx, const int by) { return ax * by - ay * bx; }
...
static void solution2(const char *const input, char *const output) {
  ...
  for (int j = 0; j < rows; j++) {
    for (int i = 0; i < cols; i++) {
      if (map[INDEX(rows, cols, j, i)] == '#' && !(x == i && y == j) && visible(x, y, i, j, map, rows, cols)) {
