...
#define INDEX(str) ((str[0]-'A')*MATERIALS_BASE_4+                      \
                    (str[1]-'A')*MATERIALS_BASE_3+                      \
                    (str[2]-'A')*MATERIALS_BASE_2+                      \
                    (str[3]-'A')*MATERIALS_BASE_1+                      \
                    (str[4]-'A')*MATERIALS_BASE_0)

#ifdef DEBUG
#define UNINDEX(str,num) do {                                           \
...
#else
#define UNINDEX(str,num) do{}while(0)
#endif

#define CEILDIV(a,b) ((a+b-1)/b)

#define EXPECT_INPUT(i, exp, got) ASSERT(exp == got, "Unexpected input: Character %lu. Expected '%c' (%d) but got '%c' (%d)", i, exp, exp, got, got)
...
static void parse_ingridient(const char *const input, size_t *const i,
                             uint32_t *const ingridient, unsigned *const quantity) {
        *quantity = 0;
...
static void create(long amount, uint32_t material) {
        ...
        ASSERT(recipes->total == 1,
               "Expected all materials to have only one recipe (offending material: %s)", tmp);
