...
static int get_cache2(unsigned positions[4], unsigned acquired_keys) {
  ...
  if (e.positions[0] != positions[0] || e.positions[1] != positions[1] || e.positions[2] != positions[2] ||
      e.positions[3] != positions[3] || e.acquired_keys != acquired_keys) {
    return -1;
  } else if (e.distance == 0) {
    return -1;
  } else if (e.distance == -1) {
...
static bool get_precalc_reachable(unsigned reachable[TOTALPOSITIONS][TOTALPOSITIONS], unsigned from, unsigned to,
                                  unsigned keys) {
...
static bool get_precalc_reachable2(unsigned reachable[TOTALPOSITIONS2][TOTALPOSITIONS2], unsigned from, unsigned to,
                                   unsigned keys) {
  unsigned needed = reachable[from][to];
  for (int key = 0; key < TOTALKEYS; key++) {
    if (needed & 1 << key && !(keys & 1 << key)) {
...
static unsigned get_precalc_distance(unsigned distances[TOTALPOSITIONS][TOTALPOSITIONS], unsigned from, unsigned to) {
  return distances[from][to];
}

static unsigned get_precalc_distance2(unsigned distances[TOTALPOSITIONS2][TOTALPOSITIONS2], unsigned from,
                                      unsigned to) {
  return distances[from][to];
}

static unsigned best_distance_to_get_keys(unsigned distances[TOTALPOSITIONS][TOTALPOSITIONS],
                                          unsigned reachable[TOTALPOSITIONS][TOTALPOSITIONS], unsigned position,
                                          unsigned acquired_keys, unsigned num_acquired_keys) {
  ...
  for (unsigned i = 0; i < TOTALKEYS; i++) {
    if (!(acquired_keys & 1 << i) && get_precalc_reachable(reachable, position, i, acquired_keys)) {
      unsigned distance = get_precalc_distance(distances, position, i);

      distance += best_distance_to_get_keys(distances, reachable, i, acquired_keys | 1 << i, num_acquired_keys + 1);
...
static unsigned best_distance_to_get_keys2(unsigned distances[TOTALPOSITIONS2][TOTALPOSITIONS2],
                                           unsigned reachable[TOTALPOSITIONS2][TOTALPOSITIONS2], unsigned positions[4],
                                           unsigned acquired_keys, unsigned num_acquired_keys) {
  if (num_acquired_keys == TOTALKEYS) {
    return 0;
  }
  ...
  for (unsigned i = 0; i < TOTALKEYS; i++) {
    for (unsigned j = 0; j < 4; j++) {
      if (!(acquired_keys & 1 << i) && get_precalc_reachable2(reachable, positions[j], i, acquired_keys)) {
        ...
        distance += best_distance_to_get_keys2(distances, reachable, new_positions, acquired_keys | 1 << i,
                                               num_acquired_keys + 1);
...
__attribute__((pure)) static struct vec4 advance(struct vec4 pos, unsigned dir) {
  if (dir == 0) {
    pos.y--;
  } else if (dir == 1) {
    pos.x++;
...
static void bfs(char maze[MAZESIZE][MAZESIZE], struct vec2 p1, struct vec2 p2, unsigned *distance, unsigned *keys) {
...
static void precalculate(char maze[MAZESIZE][MAZESIZE], unsigned distances[TOTALPOSITIONS][TOTALPOSITIONS],
                         unsigned reachable[TOTALPOSITIONS][TOTALPOSITIONS], struct vec2 initial_position,
                         struct vec2 key_positions[TOTALKEYS]) {
...
static void precalculate2(char maze[MAZESIZE][MAZESIZE], unsigned distances[TOTALPOSITIONS2][TOTALPOSITIONS2],
                          unsigned reachable[TOTALPOSITIONS2][TOTALPOSITIONS2], struct vec2 initial_positions[4],
                          struct vec2 key_positions[TOTALKEYS]) {
  for (unsigned k1 = 0; k1 < TOTALKEYS; k1++) {
    for (unsigned k2 = 0; k2 < TOTALKEYS; k2++) {
      unsigned distance, keys;
...
static void find_positions(char maze[MAZESIZE][MAZESIZE], struct vec2 *initial_position,
                           struct vec2 key_positions[TOTALKEYS]) {
...
static void adjust_part2(char maze[MAZESIZE][MAZESIZE], struct vec2 initial_position, struct vec2 robot_positions[4]) {
...
static void solution2(const char *const input, char *const output) {
  ...
  DBG("Runing main recusive algorithm...");
  unsigned robot_encoded_positions[4] = {INITIALPOSITION1, INITIALPOSITION2, INITIALPOSITION3, INITIALPOSITION4};
