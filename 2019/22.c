...
static __int128 index(__int128 i) { return deck[i]; }
...
__attribute__((const)) static __int128 modpow(__int128 base, __int128 exponent, __int128 modulus) {
  if (modulus == 1) {
    return 0;
  }
...
static void cut_cards(__int128 n) { start = MOD(start + increment * n, deck_size); }

static void deal_with_increment(__int128 n) {
  increment = MOD(increment * modpow(n, deck_size - 2, deck_size), deck_size);
}

static __int128 index(__int128 i) { return MOD(start + i * increment, deck_size); }
