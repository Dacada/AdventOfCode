...
static int grid_idx(struct coord coords, const struct coord dims) {
  ...
  return coords.x + coords.y * dims.x + coords.z * dims.y * dims.x + coords.w * dims.z * dims.y * dims.x +
...
static void free_grid(struct grid *const grid) { free(grid->grids); }

static bool grid_get(const struct grid *const grid, const struct coord coords) {
  if (ABS(coords.x) >= grid->dims.x || ABS(coords.y) >= grid->dims.y || ABS(coords.z) >= grid->dims.z ||
      ABS(coords.w) >= grid->dims.w) {
    return false;
  }
