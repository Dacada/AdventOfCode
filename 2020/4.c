...
enum fields {
        byr, iyr, eyr, hgt, hcl, ecl, pid, cid, last
};
char *fieldsstr[last] = {
...
static bool val_byr(const char *const content, const size_t len) {
        return val_year(content, len, 1920, 2002);
}
static bool val_iyr(const char *const content, const size_t len) {
        return val_year(content, len, 2010, 2020);
}
static bool val_eyr(const char *const content, const size_t len) {
        return val_year(content, len, 2020, 2030);
}
...
static bool val_ecl(const char *const content, const size_t len) {
        ...
        return strncmp(content, "amb", len) == 0 ||
                strncmp(content, "blu", len) == 0 ||
                strncmp(content, "brn", len) == 0 ||
                strncmp(content, "gry", len) == 0 ||
                strncmp(content, "grn", len) == 0 ||
                strncmp(content, "hzl", len) == 0 ||
