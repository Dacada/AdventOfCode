...
static void set_sort(struct set *const set) { qsort(set->elements, set->count, sizeof *set->elements, strcmp_wrapper); }
      ...
      if (!set_find_linear(&seen_allergens, allergen, &k)) {
        ...
        if (allergen_ingredients_count >= allergen_ingredients_capacity) {
          ...
          allergen_ingredients =
              realloc(allergen_ingredients, allergen_ingredients_capacity * sizeof *allergen_ingredients);
        }
      ...
      for (size_t i = 0; i < allergen_ingredients_count; i++) {
        ...
        if (!decided_ingredients[i]) {
          if (allergen_ingredients[i].count == 1) {
            ...
            DBG("%s's ingredient can only be %s", allergen, ingredient);

            for (size_t j = 0; j < allergen_ingredients_count; j++) {
              if (i != j) {
                set_remove(allergen_ingredients + j, allergen_ingredients[i].elements[0]);
              }
            }
          } else {
