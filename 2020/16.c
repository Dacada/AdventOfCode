...
static void free_ticket(const struct ticket *const t) { free(t->fields); }
...
__attribute__((pure)) static bool follows_rule(const int field, const struct rule *const rule) {
    ...
    for (size_t j = 0; j < nfields; j++) {
      ...
      if (candidate_count == 1) {
        DBG("Field %lu has only one candidate rule (%lu). "
            "It's removed from all other fields.",
            j, which_rule);
        if (!decided_fields[j]) {
