...
static void memory_set(struct memory_tree *const memory, unsigned long *total, uint64_t address, uint64_t value) {
...
static bool parse_input_2(const char *const input, size_t *const i, char mask[MASKSIZE + 1], uint64_t *const address,
...
static void apply_mask(uint64_t address, const char *mask, uint64_t *const addresses, unsigned *const addresses_size) {
  *addresses_size = 1;
  addresses[0] = 0;
...
static void memory_set_2(struct memory_tree *const memory, const char mask[MASKSIZE], const uint64_t address,
                         const uint64_t value, unsigned long *const total) {
...
__attribute__((pure)) static unsigned long memory_tree_sum(const struct memory_tree *const tree) {
  if (tree->empty) {
    return 0;
  }
