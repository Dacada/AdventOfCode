...
static void solution(const char *const input, char *const output, unsigned goal) {
  ...
  for (; turn <= goal; turn++) {
    ...
    if (numbers[last] == 0) {
      number = 0;
      DBG("The %uth number spoken is 0 as the last one, %d, had not been seen before", turn, last);
    } else {
...
static void solution1(const char *const input, char *const output) { solution(input, output, 2020); }

static void solution2(const char *const input, char *const output) { solution(input, output, 30000000); }
