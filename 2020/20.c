...
static void image_displace(struct image *const image, const enum direction dir) {
        ...
        for (int j=0; j<image->side; j++) {
                for (int i=0; i<image->side; i++) {
                        ...
                        newtiles[idx] = NULL;
                        if (i+diffx >= image->side || j+diffy >= image->side ||
                            i+diffx < 0 || j+diffy < 0) {
                                continue;
                        }
...
static void get_border(const struct tile *const tile, const struct transform transform,
                       const enum direction direction, char *const border) {
        // a big fat hardcoded table sounds like the easiest and fastest way to do it

        ASSERT(transform.flip != FLIP_LAST && transform.rotation != ROTATION_LAST,
               "unexpected flip or rotation %d", tile->id);
...
static bool fit_frame(const struct tile *const tiles[4], const struct transform transforms[4],
        ...
        for (int i=0; i<4; i++) {
                if (tiles[i] != NULL) {
                        ...
                        //DBG("Trying to match %d, possible neighbor %d has %c%c%c%c%c%c%c%c%c%c border in direction %s", newtile->id, tiles[i]->id, goals[i][0], goals[i][1], goals[i][2], goals[i][3], goals[i][4], goals[i][5], goals[i][6], goals[i][7], goals[i][8], goals[i][9], (const char *[]){"UP","RIGHT","DOWN","LEFT"}[(i+2)%4]);
                }
        }
        ...
        for (enum rotation rot=ROTATION_NONE; rot<ROTATION_LAST; rot++) {
                for (enum flip flip=FLIP_NONE; flip<FLIP_LAST; flip++) {
                        ...
                        //DBG("Try transform: %s rot %s flip", (const char *[]){"no","clock","half","ctrclock"}[possible_transform.rotation], (const char *[]){"no","horiz"}[possible_transform.flip]);
                        ...
                        for (int i=0; i<4; i++) {
                                ...
                                if (strncmp(goals[i], this_border, TILE_SIDE) != 0) {
                                        ...
                                        //DBG("%c%c%c%c%c%c%c%c%c%c (%d) (%s) == %c%c%c%c%c%c%c%c%c%c (%d) (%s)", goals[i][0], goals[i][1], goals[i][2], goals[i][3], goals[i][4], goals[i][5], goals[i][6], goals[i][7], goals[i][8], goals[i][9], tiles[i]->id, (const char *[]){"UP","RIGHT","DOWN","LEFT"}[(i+2)%4], this_border[0], this_border[1], this_border[2], this_border[3], this_border[4], this_border[5], this_border[6], this_border[7], this_border[8], this_border[9], newtile->id, (const char *[]){"UP","RIGHT","DOWN","LEFT"}[i]);
                                }
                        }
...
static bool find_position(const struct image *const image, const struct tile *const tile,
                          int *const x, int *const y, struct transform *const transform) {
...
static bool match_pattern(char *const bigimg, int x, int y, int side,
                          const char *const pattern, const int px, const int py) {
        bool match = true;
