...
static void stack_init(struct stack *stack) { stack->next = 0; }
...
static bool stack_empty(const struct stack *const stack) { return stack->next == 0; }
...
__attribute__((pure)) static long evaluate1(const struct token *token) {
...
__attribute__((pure)) static long evaluate2(const struct token *token) {
  struct stack operators;
  struct stack operands;
    ...
    case OPERATOR:
      if (token->value == '(') {
        ...
        if (stack_empty(&operators)) {
          ...
          if (tos.value == '(' || (token->value == '*' && tos.value == '*') ||
              (token->value == '+' && tos.value == '+') || (token->value == '+' && tos.value == '*')) {
            stack_push(&operators, *token);
...
static void solution1(const char *const input, char *const output) { solution(input, output, evaluate1); }

static void solution2(const char *const input, char *const output) { solution(input, output, evaluate2); }
